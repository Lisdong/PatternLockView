...
public class PatternLockView extends View {
    ...
    private int mcorrectStateColorSide;
    private int mWrongStateColorSide;
    ...
    public @interface PatternViewMode {
         ...
         * 此状态表示用户正确绘制的图案。路径的颜色和这两个点都会变成这个颜色。
         * <p>
         ...
         * the dots both would be changed to this color.
         * 此状态表示用户错误绘制的模式。路径的颜色和这两个点都会变成这个颜色。
         * <p>
     ...
     * 锁定模式的每个圆的动画所花费的时间（毫秒），如果设置动画模式。整个动画应该采用这个常量要完成的模式的长度。
     */
    private static final int MILLIS_PER_CIRCLE_ANIMATING = 700;

    // Amount of time (in millis) spent to animate a dot   为标志点
    private static final int DEFAULT_DOT_ANIMATION_DURATION = 190;
    // Amount of time (in millis) spent to animate a path ends为在结束动画路径
    private static final int DEFAULT_PATH_END_ANIMATION_DURATION = 100;
    // This can be used to avoid updating the display for very small motions or noisy panels这可用于避免更新显示屏以适应非常小的运动或嘈杂的面板。
    ...
    // Made static so that the static inner class can use it使其成为静态的，以便静态内部类可以使用它
    ...
    // The pattern represented as a list of connected {@link Dot}链接点列表表示的模式
     ...
     * progress animation.
     * 我们当前正在绘制的图案点的查找表。
     * 这将是完整图案的点，除非我们正在制作动画，
     * 在这种情况下，我们用这个来保存我们正在绘制的点。
     * 进度动画。
     */
    ...
    private boolean mEnableHapticFeedback = false;//触摸反馈
    ...
    public PatternLockView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mcorrectStateColorSide = typedArray.getColor(R.styleable.PatternLockView_correctStateColor_side,
                    ResourceUtils.getColor(getContext(), R.color.white));
            mWrongStateColorSide = typedArray.getColor(R.styleable.PatternLockView_wrongStateColor_side,
                    ResourceUtils.getColor(getContext(), R.color.pomegranate));
        ...
        // The pattern will always be symmetrical图案总是对称的
    ...
    private void initView() {
        ...
        mPathPaint.setColor(mCorrectStateColor);
     ...
     * 显式设置模式，而不是等待用户输入模式。您可以将其用于帮助或演示目的。
     * @param patternViewMode The mode in which the pattern should be displayed显示模式的模式
     ...
     * 设置视图是否处于隐藏模式。如果@代码为真，则
     * 当用户进入模式时，没有可见的反馈（路径绘制、点动画等）
     */
     ...
     * an unwanted state
     * 启用/禁用视图的任何用户输入。这对于临时锁定视图很有用
     * 在向用户显示任何消息以便用户无法获取视图时
     * 不需要的状态
     */
    ...
    private void startDotSelectedAnimation(final Dot dot) {
...
//                });
    ...
    private void handleActionMove(MotionEvent event) {
        ...
        for (int i = 0; i < historySize + 1; i++) {
            ...
            // Note current x and y for rubber banding of in progress patterns当前X和Y用于进行中模式的橡胶带
    ...
    private void handleActionUp(MotionEvent event) {
        // Report pattern detected检测到报告模式
        if (!mPattern.isEmpty()) {
            mPatternInProgress = false;
    ...
    private void drawCircle(Canvas canvas, float centerX, float centerY, float size, boolean partOfPattern, float alpha) {
        ...
        mDotPaint.setAlpha((int) (alpha * 255));
            ...
            if (!partOfPattern || mInStealthMode ) {
                ...
                //mWrongStateColor;
//                mDotPaint.setColor(0xFFFCF6F6);
                mDotPaint.setColor(mWrongStateColorSide);
                mDotPaint.setStrokeWidth(40);
                canvas.drawCircle(centerX, centerY, size+60 , mDotPaint);
                mDotPaint.setColor(mWrongStateColor);
                canvas.drawCircle(centerX, centerY, size , mDotPaint);
            } else if (mPatternViewMode == CORRECT || mPatternViewMode == AUTO_DRAW) {
                // mCorrectStateColor;
//                mDotPaint.setColor(0xFFF4F7FE);
                mDotPaint.setColor(mcorrectStateColorSide);
                mDotPaint.setStrokeWidth(40);
                canvas.drawCircle(centerX, centerY, size+60 , mDotPaint);
                mDotPaint.setColor(mCorrectStateColor);
                canvas.drawCircle(centerX, centerY, size , mDotPaint);
            }else {
    ...
    public static class Dot implements Parcelable {
        ...
        /**
         * Gets a cell from its identifier从其标识符获取单元格
         */
